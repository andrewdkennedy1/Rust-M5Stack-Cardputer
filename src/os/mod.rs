...
fn menu_action_to_key(action: MenuAction) -> Option<crate::keyboard::Key> {
    ...
    match action {
        ...
        MenuAction::Refresh => Some(Key::Fn),
        MenuAction::TabNext => Some(Key::Tab),
    }
}
...
pub fn boot() -> ! {
    ...
    unsafe {
        ...
        if !partition.is_null()
            && (*partition).type_ == sys::esp_partition_type_t_ESP_PARTITION_TYPE_APP
            && (*partition).subtype
    ...
    // Auto-mount SD
    render_status(&mut buffers, "Boot", &["Mounting SD..."], None);
    ...
    std::thread::spawn(move || {
        ...
        for line in stdin.lock().lines().flatten() {
            let cmd = match line.trim() {
                "RECOVERY" => RemoteCommand::FlashBin(PathBuf::from("__FACTORY__")),
                "REBOOT" => RemoteCommand::FlashBin(PathBuf::from("__REBOOT__")),
                _ => continue,
            };
